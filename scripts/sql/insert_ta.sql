...
BEGIN
   ...
   IF (NOT EXISTS(SELECT 1 from agencyuser.agency_user where (agency_id=in_agency_id AND user_name=var_email and user_type='TA'))) THEN
     ...
     VALUES (new_user_id,in_agency_id,var_email,var_email,3,fname,mname,lname,'t',now(),'t',now(),'0','f','A',now(),now(),brand,'t','RA');
...
SELECT agencyuser.add_TA('test4@test.com', 'Donald','','Duck', '6');
